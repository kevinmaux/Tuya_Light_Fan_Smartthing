-- src/aes.lua
-- Full Pure-Lua AES-128-ECB with PKCS7 Padding for Tuya Devices
local aes = {}

local SBOX = {
  
}

local INV_SBOX = {
  
}

local RCON = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36}

local function gmul(a, b)
  local p = 0
  for _ = 1, 8 do
    if (b & 1) ~= 0 then p = p ~ a end
    local hi = (a & 0x80) ~= 0
    a = (a << 1) & 0xFF
    if hi then a = a ~ 0x1b end
    b = b >> 1
  end
  return p
end

local function key_expansion(key)
  local w = {}
  for i = 1, #key do w[i] = string.byte(key, i) end
  for i = 17, 176, 4 do
    local t = {w[i-4], w[i-3], w[i-2], w[i-1]}
    if (i - 1) % 16 == 0 then
      t = {SBOX[t[2]+1] ~ RCON[(i-1)//16 + 1], SBOX[t[3]+1], SBOX[t[4]+1], SBOX[t[1]+1]}
    end
    w[i]   = w[i-16] ~ t[1]
    w[i+1] = w[i-15] ~ t[2]
    w[i+2] = w[i-14] ~ t[3]
    w[i+3] = w[i-13] ~ t[4]
  end
  return w
end

local function add_round_key(state, w, round)
  for i = 1, 16 do state[i] = state[i] ~ w[(round * 16) + i] end
end

local function sub_bytes(state)
  for i = 1, 16 do state[i] = SBOX[state[i] + 1] end
end

local function inv_sub_bytes(state)
  for i = 1, 16 do state[i] = INV_SBOX[state[i] + 1] end
end

local function shift_rows(state)
  state[1], state[5], state[9], state[13] = state[1], state[5], state[9], state[13]
  state[2], state[6], state[10], state[14] = state[6], state[10], state[14], state[2]
  state[3], state[7], state[11], state[15] = state[11], state[15], state[3], state[7]
  state[4], state[8], state[12], state[16] = state[16], state[4], state[8], state[12]
end

local function inv_shift_rows(state)
  -- FIX: this must right-rotate each row by its shift amount (0/1/2/3) to
  -- undo shift_rows' left-rotation. The previous version instead swapped
  -- the 2nd/4th element of every row and left the 1st/3rd untouched --
  -- which isn't a valid rotation for rows 0, 1, or 3 (only row 2's shift
  -- amount of 2 happens to be self-inverse). Verified against shift_rows
  -- with a round-trip test using 16 distinct marker values.
  state[1], state[5], state[9], state[13]  = state[1], state[5], state[9], state[13]
  state[2], state[6], state[10], state[14] = state[14], state[2], state[6], state[10]
  state[3], state[7], state[11], state[15] = state[11], state[15], state[3], state[7]
  state[4], state[8], state[12], state[16] = state[8], state[12], state[16], state[4]
end

local function mix_columns(state)
  for i = 0, 3 do
    local col = i * 4
    local s0, s1, s2, s3 = state[col+1], state[col+2], state[col+3], state[col+4]
    state[col+1] = gmul(0x02, s0) ~ gmul(0x03, s1) ~ s2 ~ s3
    state[col+2] = s0 ~ gmul(0x02, s1) ~ gmul(0x03, s2) ~ s3
    state[col+3] = s0 ~ s1 ~ gmul(0x02, s2) ~ gmul(0x03, s3)
    state[col+4] = gmul(0x03, s0) ~ s1 ~ s2 ~ gmul(0x02, s3)
  end
end

local function inv_mix_columns(state)
  for i = 0, 3 do
    local col = i * 4
    local s0, s1, s2, s3 = state[col+1], state[col+2], state[col+3], state[col+4]
    state[col+1] = gmul(0x0e, s0) ~ gmul(0x0b, s1) ~ gmul(0x0d, s2) ~ gmul(0x09, s3)
    state[col+2] = gmul(0x09, s0) ~ gmul(0x0e, s1) ~ gmul(0x0b, s2) ~ gmul(0x0d, s3)
    state[col+3] = gmul(0x0d, s0) ~ gmul(0x09, s1) ~ gmul(0x0e, s2) ~ gmul(0x0b, s3)
    state[col+4] = gmul(0x0b, s0) ~ gmul(0x0d, s1) ~ gmul(0x09, s2) ~ gmul(0x0e, s3)
  end
end

local function aes_encrypt_block(block, w)
  local state = {table.unpack(block)}
  add_round_key(state, w, 0)
  for round = 1, 9 do
    sub_bytes(state)
    shift_rows(state)
    mix_columns(state)
    add_round_key(state, w, round)
  end
  sub_bytes(state)
  shift_rows(state)
  add_round_key(state, w, 10)
  return state
end

local function aes_decrypt_block(block, w)
  local state = {table.unpack(block)}
  add_round_key(state, w, 10)
  inv_shift_rows(state)
  inv_sub_bytes(state)
  for round = 9, 1, -1 do
    add_round_key(state, w, round)
    inv_mix_columns(state)
    inv_shift_rows(state)
    inv_sub_bytes(state)
  end
  add_round_key(state, w, 0)
  return state
end

function aes.encrypt_ecb(data, key)
  if #key ~= 16 then error("Key must be 16 bytes for AES-128") end
  local pad = 16 - (#data % 16)
  data = data .. string.char(pad):rep(pad)
  local w = key_expansion(key)
  local out = {}
  for b = 1, #data, 16 do
    local block = {string.byte(data, b, b + 15)}
    local encrypted_block = aes_encrypt_block(block, w)
    for i = 1, 16 do table.insert(out, string.char(encrypted_block[i])) end
  end
  return table.concat(out)
end

function aes.decrypt_ecb(data, key)
  if #key ~= 16 then error("Key must be 16 bytes for AES-128") end
  if #data % 16 ~= 0 then return nil end
  local w = key_expansion(key)
  local out = {}
  for b = 1, #data, 16 do
    local block = {string.byte(data, b, b + 15)}
    local decrypted_block = aes_decrypt_block(block, w)
    for i = 1, 16 do table.insert(out, string.char(decrypted_block[i])) end
  end
  return table.concat(out)
end

return aes