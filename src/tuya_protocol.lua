...
-- FIX: aes.encrypt_ecb() already applies PKCS7 padding internally.
-- Padding here too double-pads the payload: the device recovers the
-- original JSON with several trailing garbage bytes still attached
-- after it strips the padding, and the message will fail to parse.
...
local function pack_message(dev_id, local_key, dps)
  ...
  -- FIX: wrap at 2^32 (0x100000000), not 0xFFFFFFFF, so the sequence
  -- number can actually reach 0xFFFFFFFF instead of skipping it.
