...
local function decrypt_payload(payload_data, key)
  -- Strip 15-byte ASCII Protocol Version Header if present
...
-- Re-engineered to rely on the active TCP Socket
function tuya_lan.send_command(parent_device, dps, cmd_override)
  local prefs = parent_device.preferences
  local cmd = cmd_override or 7 -- 7=CONTROL, 10=DP_QUERY, 9=HEART_BEAT
...
local function socket_listener(parent_device)
  ...
  while true do
    ...
    if err == "timeout" then
      -- Maintain Tuya TCP Session requirements with Pings
    ...
    elseif header then
      ...
      if prefix == 0x000055AA then
        ...
        if remainder and #remainder == length then
          ...
          -- Decrypt STATUS (8) or DP_QUERY response (10)
...
function tuya_lan.connect(parent_device)
  ...
  tcp:settimeout(5) -- 5-second timeout yields to the heartbeat trigger
  ...
  if success then
    ...
    tuya_lan.send_command(parent_device, nil, 10) -- Request initial state
  else
